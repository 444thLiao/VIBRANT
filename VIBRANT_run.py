...
try:
	...
	if not os.path.exists(files + 'VIBRANT_AMGs.tsv'):
		logging.info("VIBRANT error: could not identify VIBRANT_AMGs.tsv in files directory.")
		exit()
	if not os.path.exists(files + 'VIBRANT_names.tsv'):
		logging.info("VIBRANT error: could not identify VIBRANT_names.tsv in files directory.")
		exit()
	if not os.path.exists(files + 'VIBRANT_machine_model.sav'):
		logging.info("VIBRANT error: could not identify VIBRANT_machine_model.sav in files directory.")
		exit()
			...
			with open(str(out_folder)+str(base)+'.parallel-runs.txt', 'w') as outfile:
				for name, seq in SimpleFastaParser(infile):
					if " # " not in name:
						...
						logging.info("VIBRANT Error: either the wrong format (-f) was set or proteins are not in Prodigal format. Check input file (-i) and format (-f).")
						logging.info("See example data file 'Microviridae_ctch16_MH552510.2.faa' for Prodigal format.")
						exit()
	...
	if format == "nucl":
		try:
			...
			subprocess.run('rm ' + str(out_folder)+str(base)+'.parallel-runs* 2>/dev/null', shell=True)
			subprocess.run('rm ' + str(out_folder)+str(base)+'_four-orf-count.txt 2>/dev/null', shell=True)
			logging.info('No phages found. There were either no scaffolds at least ' + str(lim_low) + 'bp or ' + str(orf_low) + ' ORFs (set minimum size), or the input file (-i) format (-f) was not FASTA nucleotide. Exiting.')
			exit()
	...
	if format == "prot":
		try:
			...
			subprocess.run('rm ' + str(out_folder)+str(base)+'.parallel-runs* 2>/dev/null', shell=True)
			subprocess.run('rm ' + str(out_folder)+str(base)+'_four-orf-count.txt 2>/dev/null', shell=True)
			logging.info("No phages found. There were either no scaffolds at least " + str(orf_low) + " ORFs (set minimum size) or VIBRANT never found any proteins. Verify input file (-i) format (-f) is FASTA protein. Exiting.")
			exit()
	...
	try:
		...
		logging.info('No phages found. There were no scaffolds at least ' + str(orf_low) + ' ORFs (set minimum size). Exiting.')
		subprocess.run('rm ' + str(out_folder)+str(base)+'.parallel-runs* 2>/dev/null', shell=True)
		subprocess.run('rm ' + str(out_folder)+str(base)+'_four-orf-count.txt 2>/dev/null', shell=True)
		exit()
						...
						if "fragment" not in name:
							...
							while n <= len(seq):
								...
								if str(seq_start) == str(seq_end):
									...
									db_dict_circ.update({name:save})
									circular_list.append(name)
									n += 900
								else:
									n += 1
					for item, sequence in SimpleFastaParser(lytic_phages):
						...
						while n <= len(sequence):
							...
							if str(sequence_start) == str(sequence_end):
								...
								db_dict_circ.update({item:save})
								circular_list.append(item)
								n += 900
							else:
								n += 1
		with open(str(out_folder)+base + '.phages_circular.fna', 'w') as circ_fna:
			for item in db_dict_circ.keys():
				circ_fna.write(">" + str(item) + "\n" + db_dict_circ[item] + "\n")
	...
	cat_norm = 'cat '+str(out_folder)+'VIBRANT_normalized*' + base + '* > '+str(out_folder)+'temp_VIBRANT_normalized_' + str(base) + '.txt 2>/dev/null'
	p12 = subprocess.Popen(cat_norm, shell=True)
	...
	subprocess.Popen('rm '+str(out_folder)+'VIBRANT_normalized*' + base + '* 2>/dev/null', shell=True)
	...
	header_norm ='echo "scaffold\ttotal genes\tall KEGG\tKEGG v-score\tall Pfam\tPfam v-score\tall VOG\tVOG v-score\tKEGG int-rep\tKEGG zero\tPfam int-rep\tPfam zero\tVOG redoxin\tVOG rec-tran\tVOG int\tVOG RnR\tVOG DNA\tKEGG restriction check\tKEGG toxin check\tVOG special\tannotation check\tp_v check\tp_k check\tk_v check\tk check\tp check\tv check\th check" | cat - ' + str(out_folder)+'temp_VIBRANT_normalized_' + str(base) + '.txt > ' + str(out_folder)+'VIBRANT_summary_normalized_' + str(base) + '.tsv'
	p7 = subprocess.Popen(header_norm, shell=True)
	...
	subprocess.Popen('rm '+str(out_folder)+'temp_VIBRANT_normalized*' + base + '* 2>/dev/null', shell=True)
	...
	subprocess.Popen('mkdir -p '+ str(out_folder)+'VIBRANT_HMM_tables_unformatted_'+base, shell=True)
	subprocess.Popen('mkdir -p '+ str(out_folder)+'VIBRANT_HMM_tables_parsed_'+base, shell=True)
	subprocess.Popen('mkdir -p '+ str(out_folder)+'VIBRANT_phages_'+base, shell=True)
	subprocess.Popen('mkdir -p '+ str(out_folder)+'VIBRANT_results_'+base, shell=True)
	...
	with open(str(out_folder)+base + '.phages_combined.faa', 'r') as fasta:
		with open(str(out_folder)+'VIBRANT_integrated_prophage_coordinates_' + base + '.tsv', 'w') as coord_file:
			coord_file.write("scaffold\tfragment\tprotein start\tprotein stop\tprotein length\tnucleotide start\tnucleotide stop\tnucleotide length\n")
			for name, seq in SimpleFastaParser(fasta):
				...
				if "_fragment_" in name:
					...
					coords = name.split("\t",2)[1]
					master_coords.append(str(rename.rsplit("_",1)[0]))
					master_coords.append(str(coords.split("..")[0])[1:])
					master_coords.append(str(coords.split("..")[1])[0:-1])
					master_coords.append(str(rename.rsplit("_",1)[1]))
			master_coords.append('')
			genbank_database.extend(('',''))

			n = 0
			first = True
			while master_coords[n] != '':
				if master_coords[n] == master_coords[n+4]:
					if first == True:
						start = master_coords[n+1]
						prot = master_coords[n+3]
					first = False
				else:
					coord_file.write(str(master_coords[n]).rsplit("_",2)[0] + "\t" + str(master_coords[n]) + "\t" + str(prot) + "\t" + str(master_coords[n+3]) + "\t" + str(int(master_coords[n+3])-int(prot)+1) + "\t" + str(start) + "\t" + str(master_coords[n+2]) + "\t" + str(int(master_coords[n+2])-int(start)+1) + "\n")
					first = True
				n += 4
			...
			while n < len(genbank_database)-1:
				if check == False:
					...
					if str(genbank_database[n]) in circular_database:
						...
						form = "linear"
					genbank.write('//\nLOCUS       ' + str(genbank_database[n]) + '               ' + str(length_database[genbank_database[n]]) + ' bp    DNA     ' + str(form) + ' VRL ' + str(date.today().strftime("%d-%b-%Y")) + '\nDEFINITION  ' + str(genbank_database[n]) + '.\nCOMMENT     Annotated using VIBRANT v1.2.1\nFEATURES             Location/Qualifiers\n     source          1..' + str(length_database[genbank_database[n]]) + '\n                     /organism="' + str(genbank_database[n]) + '"\n')
					check = True
					...
					if check == True:
						...
						if str(genbank_database[n+2]) == "-1":
							if str(genbank_database[n]) == str(genbank_database[n+7]):
								genbank.write('     CDS             complement(' + str(location) + ')' + '\n                     /locus_tag="' + str(genbank_database[n-1]).replace('"','') + '"\n                     /protein_id="' + str(genbank_database[n+3]).replace('"','') + '"\n                     /product="' + str(genbank_database[n+4]).replace('"','') + '"\n                     /translation="' + str(genbank_database[n+5]).replace('"','') + '"\n')
							if str(genbank_database[n]) != str(genbank_database[n+7]):
								genbank.write('     CDS             complement(' + str(location) + ')' + '\n                     /locus_tag="' + str(genbank_database[n-1]).replace('"','') + '"\n                     /protein_id="' + str(genbank_database[n+3]).replace('"','') + '"\n                     /product="' + str(genbank_database[n+4]).replace('"','') + '"\n                     /translation="' + str(genbank_database[n+5]).replace('"','') + '"\n')
	...
	if suppress == False and int(out_phages) > 0:
		...
		with open(str(out_folder)+'VIBRANT_genome_quality_' + base + '.tsv', 'r') as qual_file:
			...
			qualities = data_frame['Quality'].tolist()
			lowqual = qualities.count("low quality draft")
			mediumqual = qualities.count("medium quality draft")
			highqual = qualities.count("high quality draft")
			if format == 'nucl':
				circqual = qualities.count("complete circular")
				data = [lowqual,  mediumqual, highqual, circqual]
				names = ['low quality\n(' + str(data[0]) + ')', 'medium quality\n(' + str(data[1]) + ')', 'high quality\n(' + str(data[2]) + ')', 'circular\n(' + str(data[3]) + ')']
			if format == 'prot':
				data = [lowqual,  mediumqual, highqual]
				names = ['low quality\n(' + str(data[0]) + ')', 'medium quality\n(' + str(data[1]) + ')', 'high quality\n(' + str(data[2]) + ')']
			qual_fig = sns.barplot(x=names, y=data, alpha=0.6, palette=sns.color_palette("pastel"), edgecolor=sns.color_palette("bright"))
			plt.ylabel('Count')
			plt.tight_layout()
	...
	if suppress == False and int(out_phages) > 2:
		plt.figure()
		lifestyle_list = []
		quality_list = []
		circ_list = []
		label_list = []
		with open(str(out_folder)+'VIBRANT_genome_quality_' + base + '.tsv', 'r') as qual_file:
			qual_file = qual_file.read().replace("\n","\t").split("\t")
			x = 3
			lq = 0
			hq = 0
			mq = 0
			...
			try:
				cl = len(circular_list)
			except Exception:
				cl = 0
			while x < len(qual_file)-1-(cl*3):
				if str(qual_file[x+2]) == 'low quality draft':
					quality_list.append((0.9,0,0.1,0.5))
					label_list.append('low quality draft')
					lq += 1
				elif str(qual_file[x+2]) == 'medium quality draft':
					quality_list.append((0,0.2,0.9,0.5))
					label_list.append('medium quality draft')
					mq += 1
				elif str(qual_file[x+2]) == 'high quality draft':
					quality_list.append((0.2,0.9,0,0.5))
					label_list.append('high quality draft')
					hq += 1
				if format == 'nucl':
					if str(qual_file[x]) in circular_list:
						circ_list.append('dotted')
						circ += 1
					else:
						circ_list.append('solid')
						lin += 1
				if str(qual_file[x+1]) == 'lytic':
					lifestyle_list.append((0,0,0,1))
					lyt += 1
				elif str(qual_file[x+1]) == 'lysogenic':
					lifestyle_list.append((0.85,0.85,0.85,1))
					lyso += 1
				x += 3

		annotations = pd.read_csv(str(out_folder)+'VIBRANT_summary_normalized_'+base+'.tsv', sep='\t', header=0)
		anno = pd.DataFrame(annotations, columns=['total genes', 'all KEGG' , 'KEGG v-score' , 'all Pfam' , 'Pfam v-score' , 'all VOG' , 'VOG v-score' , 'KEGG int-rep' , 'KEGG zero' , 'Pfam int-rep' , 'Pfam zero' , 'VOG redoxin' , 'VOG rec-tran' , 'VOG int' , 'VOG RnR' , 'VOG DNA' , 'KEGG restriction check', 'KEGG toxin check', 'VOG special', 'annotation check' , 'p_v check' , 'p_k check' , 'k_v check' , 'k check' , 'p check' , 'v check' , 'h check'])

		size_list = anno['total genes'].tolist()
		size_list = [10**i for i in size_list]

		anno[['scaffold']] = annotations[['scaffold']]
		anno_cols = anno.drop(['scaffold', 'total genes'], axis='columns')

		pca = PCA(n_components=2)
		principalComponents = pca.fit_transform(anno_cols)
		principalDf = pd.DataFrame(data = principalComponents, columns = ['principal component 1', 'principal component 2'])
		finalDf = pd.concat([principalDf, anno[['scaffold']]], axis = 1)
		coordsDf = pd.concat([principalDf, anno[['scaffold']]], axis = 1)
		coordsDf.to_csv(str(out_folder)+'VIBRANT_figure_PCA_' + str(base) + '.tsv', index=False, sep="\t")
		x_list = principalDf['principal component 1'].tolist()
		...
		if format == "nucl":
			plot = ax.scatter(x_list, y_list, s = size_list, marker = 'h', c = quality_list, label=label_list, edgecolors=lifestyle_list, linestyle=circ_list, linewidth=0.4)
		if format == "prot":
			plot = ax.scatter(x_list, y_list, s = size_list, marker = 'h', c = quality_list, label=label_list, edgecolors=lifestyle_list, linewidth=0.4)

		a = mpatches.Patch(color=(0.9,0,0.1,0.5), label='low quality ('+str(lq)+')')
		b = mpatches.Patch(color=(0,0.2,0.9,0.5), label='medium quality ('+str(mq)+')')
		c = mpatches.Patch(color=(0.2,0.9,0,0.5), label='high quality ('+str(hq)+')')
		legend1 = plt.legend(handles=[a,b,c], title='Scaffold quality', bbox_to_anchor=(1.0, 0.9), loc='upper left', borderaxespad=0)
		ax.add_artist(legend1)

		a = mpatches.Patch(color=(0,0,0,1), label='lytic ('+str(lyt)+')')
		b = mpatches.Patch(color=(0.85,0.85,0.85,1), label='lysogenic ('+str(lyso)+')')
		legend3 = plt.legend(handles=[a,b], title='Outline color', bbox_to_anchor=(1.0, 0.6), loc='center left', borderaxespad=0)
		ax.add_artist(legend3)

		lines = [Line2D([0], [0], color="darkgray", marker='h',markersize=3, markerfacecolor="darkgray", label='10', alpha=0.6, markeredgecolor='white', markeredgewidth='1', lw=0), Line2D([0], [0], color="darkgray", marker='h',markersize=6, markerfacecolor="darkgray", label='25', alpha=0.6, markeredgecolor='white', lw=0), Line2D([0], [0], color="darkgray", marker='h',markersize=8.2, markerfacecolor="darkgray", label='50', alpha=0.6, markeredgecolor='white', lw=0), Line2D([0], [0], color="darkgray", marker='h',markersize=9.7,markerfacecolor="darkgray", label='75', alpha=0.6, markeredgecolor='white', lw=0)]
		legend2 = plt.legend(handles=lines, title='Size (ORFs)', bbox_to_anchor=(1.0, 0.25), loc='upper left', borderaxespad=0)
		ax.add_artist(legend2)

		if format == "nucl":
			legend4 = [Line2D([0], [0], color='black', lw=2, label='circular ('+str(circ)+')', linestyle='dotted'), Line2D([0], [0], lw=2, color='black', label='linear ('+str(lin)+')')]
			ax.legend(handles=legend4, loc='center left', bbox_to_anchor=(1.0, 0.4), borderaxespad=0, title='Outline style')
	...
	logging.info("Runtime:  " + str(round((time.time() - float(starting_time))/60,1)) + " minutes")
	logging.info("Program:  VIBRANT v1.2.1")
	logging.info("\n")
